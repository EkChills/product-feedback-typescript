...
const MobileSuggestion = ({
  ...
  return <Wrapper tabName={tabName} className="flex flex-col bg-[#FFFFFF] p-[1.5rem] rounded-lg space-y-[1rem]">
      {/* planned goes here */}
      {tabName && <div className='flex  items-center space-x-[1rem]'>
    ...
    <Link to={`/suggestions/${id}`}><h2 className="card-title-text hover:text-[#4661E6] transition-all ease-in-out">{title}</h2></Link>
    <div className=''>
    ...
    <div className="flex   items-center justify-between">
      <button onClick={increaseVote} className={`rounded-lg px-[1rem] cursor-pointer transition-all duration-300 text-[#4661E6] text-[.84rem] capitalize ${isActive? 'bg-[#4661E6]' : 'bg-[#F2F4FE] hover:bg-[#CFD7FF] '} font-[600] py-1 flex items-center space-x-[.6rem] `}>
      <svg width="10" height="7" xmlns="http://www.w3.org/2000/svg"><path d="M1 6l4-4 4 4"  className={`${isActive ? 'upvoted-svg' :'downvoted-svg'}`} stroke-width="2" fill="none" fill-rule="evenodd"/></svg>
        <span className={` ${isActive ?'text-[#ffffff]' : 'text-[#3A4374]' } text-[.8125rem] font-bold  `}>{upvotes}</span>
      </button>
